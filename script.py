...
def main():
    ...
    if choice:
       with forms.WarningBar(title='Select Floors to Copy From Model'):
        linked_floors = uidoc.Selection.PickObjects(Autodesk.Revit.UI.Selection.ObjectType.LinkedElement,
                                                   "Select Floors to Copy From Model")
        element_ids_copy = [ref.LinkedElementId for ref in linked_floors]
        link_doc = Selection.get_link_doc()
    else:
        link_doc = Selection.get_link_doc()
        linked_floors = [floor for floor in DB.FilteredElementCollector(link_doc).OfCategory(DB.BuiltInCategory.OST_Floors).WhereElementIsNotElementType()]
        element_ids_copy = []
        for floor in linked_floors:
            try:
                level_name = floor.LookupParameter('Level').AsValueString()
            except AttributeError as ex:
                pass
            else:
                if level_name == 'ROOF HIGH' or level_name == 'LEVEL B1M':
                    if 'SOG' in get_type_name(floor, link_doc):
    ...
    with db.Transaction('copying floors and their openings'):
        for floor in linked_floors:
            floorElement = link_doc.GetElement(floor.LinkedElementId)
